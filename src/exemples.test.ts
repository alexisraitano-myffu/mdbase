...
it('la démo se charge sans avertissement et calcule ses rollups', async () => {
  ...
  expect([...etat.bases.values()].map((b) => b.vues.map((v) => v.id))).toEqual([
    ['cartes', 'tableau'],
    ['tableau', 'par-statut', 'planning', 'feuille-de-route'],
  ...
  expect(etat.dashboards.map((d) => [d.id, d.avertissements, d.dashboard?.rangees.map((r) => r.blocs.length)])).toEqual([['pilotage', [], [1, 2]]])
