...
export async function proposer(modele: ModeleIA, espace: DepotEspace, demande: string, o: OptionsDemande): Promise<Proposition> {
  ...
  for (let essai = 0; ; essai++) {
    ...
    const resultats = reponse.appels.map((appel): AppelValide | ErreurProposition => {
      try {
        return validerAppel(espace.etat(), appel, { aujourdhui: o.aujourdhui }, assistant)
      } catch (e) {
    ...
    if (erreurs.length === 0) {
      ...
      if (operations.length > 0 || skills.length > 0) return { type: 'plan', plan: { operations, skills }, message, memoire }
